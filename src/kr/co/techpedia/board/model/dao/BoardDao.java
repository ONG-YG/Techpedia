...
//						+ "JOIN SPPT_STAT ON(SPPT_STAT.SPPT_STATCD = TECH_SPPT.SPPT_STATCD) ";
	...
	public ArrayList<Notice> searchNoticeBoardList(Connection conn, int currPg, int recordCountPerPage,
			String searchArea, String searchKeyword) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		ArrayList<Notice> noticeList = new ArrayList<>();
		...
		String query = "SELECT * FROM ("
						+ "SELECT SEARCH_TBL.*,"
						+ "ROW_NUMBER() OVER(ORDER BY POST_NO_F DESC) AS R_NUM "
						+ " FROM ("
							+ "SELECT NOTICE.*, "
								+ "MEMBER_NAME AS WRITER_NAME, "
								+ "NGRD_NAME, "
								+ "NVL(CMM_CNT,0) AS CMM_CNT, "
								+ "NOTICE.POST_NO AS POST_NO_F "
							+ "FROM NOTICE "
							+ "JOIN TP_MEMBER ON(TP_MEMBER.MEMBER_NO = NOTICE.NTC_WRITER) "
							+ "JOIN NTC_GRD_L ON(NOTICE.NTC_GRADECD = NTC_GRD_L.NTC_GRADECD)"
							+ "LEFT JOIN ("
									...
									+ "WHERE BRD_CODE='NTC' "
									+ "GROUP BY POST_NO) CMM "
							+ "ON (NOTICE.POST_NO = CMM.POST_NO) ) SEARCH_TBL "
						+ "WHERE "+searchArea+" LIKE ?"
						+ ") "
						+ "WHERE R_NUM BETWEEN ? AND ?";
	...
	public ArrayList<TechSharePost> searchTechShareBoardList(Connection conn, int currPg, int recordCountPerPage,
			String searchArea, String searchKeyword) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		ArrayList<TechSharePost> techSharePostL = new ArrayList<>();
		...
		String query = "SELECT * FROM ("
						...
						+ " FROM ("
							...
							+ "LEFT JOIN ("
									...
									+ "WHERE BRD_CODE='SHR' "
									+ "GROUP BY POST_NO) CMM "
							+ "ON (TECH_SHR.POST_NO = CMM.POST_NO) ) SEARCH_TBL "
							+ "WHERE "+searchArea+" LIKE ?"
						+ ") "
						+ "WHERE R_NUM BETWEEN ? AND ?";
	...
	public ArrayList<TechSupportPost> searchTechSupportBoardListByCompNo(Connection conn, int compNo, int currPg,
			int recordCountPerPage, String searchArea, String searchKeyword) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		ArrayList<TechSupportPost> techSupportPostL = new ArrayList<>();
		...
		String query = "SELECT * FROM ("
							...
							+ " FROM ("
								+ "SELECT TECH_SPPT.*, "
									...
									+ "STAT_NAME, "
									+ "NVL(CMM_CNT,0) AS CMM_CNT, "
									+ "TECH_SPPT.POST_NO AS POST_NO_F "
									+ "FROM TECH_SPPT "
									+ "JOIN TP_MEMBER M1 ON(M1.MEMBER_NO = TECH_SPPT.SPPT_WRITER) "
									+ "LEFT JOIN TP_MEMBER M2 ON(M2.MEMBER_NO = TECH_SPPT.SPPT_ENG) "
									+ "JOIN COMPANY_L ON(M1.COMP_NO = COMPANY_L.COMP_NO) "
	...
	public ArrayList<TechSupportPost> searchTechSupportBoardList(Connection conn, int currPg, int recordCountPerPage,
			String searchArea, String searchKeyword) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		ArrayList<TechSupportPost> techSupportPostL = new ArrayList<>();
						...
						+ " FROM ("
								+ "SELECT TECH_SPPT.*, "
									...
									+ "LEFT JOIN ("
											...
											+ "WHERE BRD_CODE='SPPT' "
											+ "GROUP BY POST_NO) CMM "
									+ "ON (TECH_SPPT.POST_NO = CMM.POST_NO) ) SEARCH_TBL "
								+ "WHERE "+searchArea+" LIKE ?"
								+ ") "
							+ "WHERE R_NUM BETWEEN ? AND ?";
	...
	public String getSearchPageNavi(Connection conn, 
		...
		if(board.equals("Notice")) {
			query = "SELECT COUNT(*) AS TOTALCOUNT FROM ("
						+ "SELECT * FROM "
						+ "(SELECT NOTICE.*, MEMBER_NAME AS WRITER_NAME FROM NOTICE "
						+ "JOIN TP_MEMBER ON(TP_MEMBER.MEMBER_NO = NOTICE.NTC_WRITER) )"
						+ "WHERE "+searchArea+" LIKE '%"+searchKeyword+"%')";
		}
		else if(board.equals("TechSh") && memberNo==-1) {
			query = "SELECT COUNT(*) AS TOTALCOUNT FROM ("
						+ "SELECT * FROM "
						+ "(SELECT TECH_SHR.*, MEMBER_NAME AS WRITER_NAME FROM TECH_SHR "
						+ "JOIN TP_MEMBER ON(TP_MEMBER.MEMBER_NO = TECH_SHR.SHR_WRITER) )"
						+ "WHERE "+searchArea+" LIKE '%"+searchKeyword+"%')";
		}
		else if(board.equals("TechSpp") && compNo!=-1) {
		...
		if(needPrev==true) {
			sb.append("<span id='prev_a'><img src='/img/prev.png' id='prev_img' width='20px' onclick='search("+(startNavi-1)+");'></span>");
		}
		else {
			sb.append("<a id='prev_a'><img src='/img/prev.png' id='prev_img' width='20px'></a>");
		}
		...
		for(int i=startNavi; i<=endNavi; i++) {
			if(i==currPg) {
				sb.append("<span class='page_link' onclick='search("+i+");'><B>"+i+"</B></span>");
			}
			else {
				sb.append("<span class='page_link' onclick='search("+i+");'>"+i+"</span>");
			}
		}
		
		if(needNext) {
			sb.append("<span id='prev_a'><img src='/img/next.png' id='next_img' width='20px' onclick='search("+(endNavi+1)+");'></span>");
		}
		else {
			sb.append("<a id='next_a'><img src='/img/next.png' id='next_img' width='20px'></a>");
		}
